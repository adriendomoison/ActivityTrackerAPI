...
func ErDupEntry(pErr *mysql.MySQLError) string {
	res := strings.Split(pErr.Message, "'")
	value := res[1]
	field := strings.Split(res[3], "_UNIQUE")[0]
	if field == "name" {
		return translate.T("sqlErDupEntry", map[string]interface{}{
			"Value": value,
			"Field": translate.T(field),
		})
	}
	return translate.T("alreadyExiste")
}
